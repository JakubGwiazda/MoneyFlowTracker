...
- `./src/components` - Client-side components written in Astro (static) and Angular (dynamic)
...
- Implement framework components in Angular only when interactivity is needed
...
- Generate unique IDs for accessibility attributes using a utility or service (e.g., `inject(DOCUMENT)`)  
- Apply **optimistic UI updates** using local component state or RxJS streams for better UX in async actions  
- Leverage **Zone-less change detection** (`provideZoneChangeDetection({ eventCoalescing: true })`) in newer Angular versions for improved responsiveness  
